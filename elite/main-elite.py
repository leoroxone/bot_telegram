...
csv_filename = 'elite/mensagens.csv'
...
# Configurações
url_canal_origem = 'https://web.telegram.org/a/#-1001752409207' 
...
with open('elite/output.html', 'w', encoding='utf-8') as file:
    file.write(div_html)

def extrair_informacoes(html):
    soup = BeautifulSoup(html, 'html.parser')

    with open('elite/indice_bruto.csv', 'w', newline='', encoding='utf-8') as csvfile:
...
with open('elite/output.html', 'r', encoding='utf-8') as file:
    codigo_html = file.read()

extrair_informacoes(codigo_html)

input_file = 'elite/indice_bruto.csv'
output_file = 'elite/indice.csv'
...
os.remove('elite/indice_bruto.csv')
...
# Criar uma pasta para salvar as elite/imagens
if not os.path.exists('elite/imagens'):
    os.makedirs('elite/imagens')

# Abrir o arquivo CSV e criar um novo arquivo CSV para a saída
with open('elite/indice.csv', 'r') as input_file, open('elite/mensagens.csv', 'w', newline='') as output_file:
    ...
    for row in reader:
        ...
        if link_img:
            ...
            local = salvar_imagem(html_content, 'elite/imagens')
...
os.remove('elite/output.html')
os.remove('elite/indice.csv')
...
async def enviar_mensagens(telegram_token, grupo_id, dados_csv, ultima_data_message_id):
    ...
    if data_message_id and int(data_message_id) > int(ultima_data_message_id):
        await bot.send_message(chat_id=grupo_id, text='Atenção - Entradas do Grupo Elite Europeia')
