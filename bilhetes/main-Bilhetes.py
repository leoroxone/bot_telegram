...
csv_filename = 'bilhetes/mensagens.csv'
...
with open('bilhetes/output.html', 'w', encoding='utf-8') as file:
    file.write(div_html)

def extrair_informacoes(html):
    soup = BeautifulSoup(html, 'html.parser')

    with open('bilhetes/indice_bruto.csv', 'w', newline='', encoding='utf-8') as csvfile:
...
with open('bilhetes/output.html', 'r', encoding='utf-8') as file:
    codigo_html = file.read()

extrair_informacoes(codigo_html)

input_file = 'bilhetes/indice_bruto.csv'
output_file = 'bilhetes/indice.csv'
...
os.remove('bilhetes/indice_bruto.csv')
...
if not os.path.exists('bilhetes/imagens'):
    os.makedirs('bilhetes/imagens')

# Abrir o arquivo CSV e criar um novo arquivo CSV para a saída
with open('bilhetes/indice.csv', 'r') as input_file, open('bilhetes/mensagens.csv', 'w', newline='') as output_file:
    ...
    for row in reader:
        ...
        if link_img:
            ...
            local = salvar_imagem(html_content, 'bilhetes/imagens')
...
os.remove('bilhetes/output.html')
os.remove('bilhetes/indice.csv')
...
async def enviar_mensagens(telegram_token, grupo_id, dados_csv, ultima_data_message_id):
    ...
    if data_message_id and int(data_message_id) > int(ultima_data_message_id):
        await bot.send_message(chat_id=grupo_id, text='Atenção - Entradas do Grupo Bilhetes')
...
arquivo = 'bilhetes/mensagens.csv'
...
try:
    shutil.rmtree('bilhetes/imagens')
