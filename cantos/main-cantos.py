...
csv_filename = 'cantos/mensagens.csv'
...
# Configurações
url_canal_origem = 'https://web.telegram.org/a/#-1001721591405' 
...
with open('cantos/output.html', 'w', encoding='utf-8') as file:
    file.write(div_html)

def extrair_informacoes(html):
    soup = BeautifulSoup(html, 'html.parser')

    with open('cantos/indice_bruto.csv', 'w', newline='', encoding='utf-8') as csvfile:
...
with open('cantos/output.html', 'r', encoding='utf-8') as file:
    codigo_html = file.read()

extrair_informacoes(codigo_html)

input_file = 'cantos/indice_bruto.csv'
output_file = 'cantos/indice.csv'
...
os.remove('cantos/indice_bruto.csv')
...
if not os.path.exists('cantos/imagens'):
    os.makedirs('cantos/imagens')

# Abrir o arquivo CSV e criar um novo arquivo CSV para a saída
with open('cantos/indice.csv', 'r') as input_file, open('cantos/mensagens.csv', 'w', newline='') as output_file:
    ...
    for row in reader:
        ...
        if link_img:
            ...
            local = salvar_imagem(html_content, 'cantos/imagens')
...
os.remove('cantos/output.html')
os.remove('cantos/indice.csv')
...
async def enviar_mensagens(telegram_token, grupo_id, dados_csv, ultima_data_message_id):
    ...
    if data_message_id and int(data_message_id) > int(ultima_data_message_id):
        await bot.send_message(chat_id=grupo_id, text='Atenção - Entradas do Grupo Cantos')
